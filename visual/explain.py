...
#	cv2.imwrite('/mnt/dive/shared/hao.yuan/tempspace2/hyuan/self_interpretation/res_all/'+str(k)+'_mk.png', np.uint8(255*mask))
# 
	np.save('/mnt/dive/shared/hao.yuan/tempspace2/hyuan/self_interpretation/results_others/'+str(k)+'_mk.npy', mask)
...
if __name__ == '__main__':
    ...
    model = load_model()
    p_dir = '/mnt/dive/shared/hao.yuan/tempspace2/hyuan/self_interpretation/results_npy/'
    for k in range(40000,50000):
        path = p_dir+str(k)+'_raw.npy'
        im = np.load(path)
        path_new = p_dir+str(k)+'_raw.png'
        imsave(path_new, im)
        print('Now process image: ', k)


        original_img = cv2.imread(path_new, 1)
        original_img = cv2.resize(original_img, (224, 224))
    # original_img = np.load(sys.argv[1])
	...
	# Convert to torch variables
        ...
        upsampled_mask = upsample(mask)
        save(upsampled_mask, original_img, blurred_img_numpy, k)
